...
class DateConverter
{
    ...
    private function _is_in_range_nep($yy, $mm, $dd)
    {
        ...
        $daysInMonth = $this->_bs[$yy-2000][intval($mm)]; //check how many days are there in selected month
        if ($dd < 1 || $dd > $daysInMonth)
        {
            return 'Error! day value can be between 1-'.$daysInMonth.' only';
        }
        return TRUE;
    }
    /**
...
//	print_r($cal->nep_to_eng(2065,8,8));s
